...
### アーキテクチャ
- U-Net構造から段階的にUperNet構造に変更：初めはU-Net構造を用いていたが、室内シーンは広域クラス（天井・床・壁）と小物クラスが共存しており、U-Netの一本道デコーダでは深い層の情報が支配的になりやすく小物クラスの認識が課題だった。そこでGAPを用いることでボトルネックにグローバルな文脈を付与しようとしていたが、全空間を単一ベクトルへ圧縮するため多スケールの情報保持に限界があると判断した。そのためUperNet構造を採用し、GAPをPPMに置き換え、複数粒度のコンテキストを同時に集約できるよう改善した。またデコーダもFPN的な構成とし、各スケールの特徴量を対等に統合することで広域・局所両方の情報を精度向上に活かした。
